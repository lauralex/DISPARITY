...
- `Tools/ReviewVerificationBaselines.ps1`: checks baseline assets for required capture/render-graph/editor-target/golden/v24-v26 coverage fields and runs performance summaries.
...
- `Tools/SummarizePerformanceHistory.ps1`: prints recent CPU/GPU and scenario trend summaries grouped by suite/executable, includes viewport/high-resolve/schema/release/preference/toolbar counters, and compares previous-run plus recent-median deltas against committed performance baselines.
...
- The `Viewport` panel persists HUD/toolbar/camera/transform/filter preferences and overlays toolbar buttons on the viewport texture: `Cam`, `Dbg`, `Cap`, `ID`, `Depth`, and `HUD`.
...
On 2026-04-27 after the v26 long-horizon editor foundation pass:
...
- `Tools/RuntimeVerifyDisparity.ps1 -Configuration Debug -Frames 90 -DisableGoldenComparison` completed successfully and reported `version=0.26.0`, `editor_preference_persistence_tests=1`, `editor_preference_save_tests=1`, `viewport_toolbar_tests=1`, and `viewport_toolbar_interactions=5`.
- `Tools/VerifyDisparity.ps1` completed successfully for v26; it now includes assetized runtime report schema assertions, ten v24 production-batch counters, forty v25 production followup point counters, v26 editor-preference persistence and viewport-toolbar counters, viewport-HUD/debug-thumbnail baseline coverage, transform precision checks, command-history filtering checks, v6 Shot Director metadata checks, VFX profile checks, cooked GPU-resource readiness checks, dependency invalidation checks, audio calibration checks, production followup manifest review, release-readiness review, cooked package review, Git-signature-aware baseline approval metadata checks, signed baseline update approval intent, trailer launch/OBS plan generation, symbol publishing, installer bootstrapper generation, and median-aware performance trend review.
...
- `Tools/ReviewVerificationBaselines.ps1 -ListGoldenProfiles` validated required baseline keys, including all v24 production-batch counters, `min_v25_production_points=40`, `min_editor_preference_persistence_tests=1`, and `min_viewport_toolbar_tests=1`, printed available golden profiles, and recognized the committed default plus known adapter profile set.
...
- `Tools/VerifyDisparity.ps1` produced PASS packaged runtime reports, captures, golden comparisons, showcase/trailer/high-res/VFX/beat/audio-analysis/Shot-Director/async-IO/animation-skinning/render-graph-callback/GPU-pick-histogram/cooked-package/nested-prefab/audio-production/viewport-overlay/high-resolve/viewport-HUD/transform-precision/command-history/schema/shot-sequencer/VFX-profile/cooked-GPU/dependency-invalidation/audio-calibration/release-readiness/v25=40/prefs=1/toolbar=1 coverage, and local performance history rows for all six suites against `dist\DISPARITY-Release\DisparityGame.exe`.
...
- The v26 pass was verified with `Tools/VerifyDisparity.ps1`; run `git status --short` and review the signed commit output before pushing.
...
- Promote editor preferences into named workspace/user profiles with reset/import/export, dock-layout persistence, schema migration, and per-project defaults.
...
- Replace the remaining high-resolution proof with true tiled offscreen rendering, per-tile camera jitter, selectable resolves, EXR output, and async compression workers.
...
- Load `DSGLBPK2` packages as optimized GPU mesh/material/animation resources, add live runtime dependency invalidation previews, rollback on failed reload, retargeting, and real GPU skinning palette uploads.
