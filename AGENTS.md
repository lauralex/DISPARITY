...
- `Tools/ReviewVerificationBaselines.ps1`: checks baseline assets for required capture/render-graph/editor-target/golden/v24-v28 coverage fields and runs performance summaries.
- `Tools/ReviewProductionBatch.ps1`: validates the v25 forty-point production followup manifest and v28 thirty-six-point diversified followup manifest, then writes review JSON under `Saved/Verification`.
...
On 2026-04-27 after the v28 diversified production batch:
...
- `Tools/RuntimeVerifyDisparity.ps1 -Configuration Debug -Frames 90 -DisableGoldenComparison` completed successfully and reported `version=0.28.0`, `editor_workflow_tests=1`, `asset_pipeline_promotion_tests=1`, `rendering_advanced_tests=1`, `runtime_sequencer_feature_tests=1`, `audio_production_feature_tests=1`, `production_publishing_tests=1`, `v28_diversified_points=36`, `production_schema_metrics=163`, and one passing `v28_point_*` metric for every diversified followup point.
- `Tools/VerifyDisparity.ps1` completed successfully for v28; it now includes assetized runtime report schema assertions, ten v24 production-batch counters, forty v25 production followup point counters, v26 editor-preference persistence and viewport-toolbar counters, v27 named profile/capture-preset/VFX-emitter/cooked-dependency counters, v28 diversified category/point counters, viewport-HUD/debug-thumbnail baseline coverage, transform precision checks, command-history filtering checks, v6 Shot Director metadata checks, VFX profile checks, cooked GPU-resource readiness checks, dependency invalidation/preview/rollback/streaming/skinning/retargeting checks, audio calibration and production-feature checks, production followup manifest review, release-readiness review, cooked package review, Git-signature-aware baseline approval metadata checks, signed baseline update approval intent, trailer launch/OBS plan generation, symbol publishing, installer bootstrapper generation, and median-aware performance trend review.
...
- `Tools/ReviewVerificationBaselines.ps1 -ListGoldenProfiles` validated required baseline keys, including all v24 production-batch counters, `min_v25_production_points=40`, `min_editor_preference_persistence_tests=1`, `min_viewport_toolbar_tests=1`, the v27 profile/capture/VFX/cooked-dependency gates, and the v28 category gates plus `min_v28_diversified_points=36`, printed available golden profiles, and recognized the committed default plus known adapter profile set.
- `Tools/ReviewProductionBatch.ps1` validated `Assets/Verification/V25ProductionBatch.dfollowups` as exactly forty uniquely named v25 points and `Assets/Verification/V28DiversifiedBatch.dfollowups` as exactly thirty-six uniquely named v28 points, then wrote `Saved/Verification/v25_production_batch_review.json` and `Saved/Verification/v28_diversified_batch_review.json`.
...
- `Tools/ReviewReleaseReadiness.ps1` produces `Saved/Release/release_readiness_manifest.json` and validates package, installer/bootstrapper, symbol, OBS, runtime schema, and v25/v28 production batch artifacts.
...
- `Tools/VerifyDisparity.ps1` produced PASS packaged runtime reports, captures, golden comparisons, showcase/trailer/high-res/VFX/beat/audio-analysis/Shot-Director/async-IO/animation-skinning/render-graph-callback/GPU-pick-histogram/cooked-package/nested-prefab/audio-production/viewport-overlay/high-resolve/viewport-HUD/transform-precision/command-history/schema/shot-sequencer/VFX-profile/VFX-emitter/cooked-GPU/dependency-invalidation/cooked-dependency-preview/audio-calibration/release-readiness/v25=40/v28=36/prefs=1/toolbar=1/profiles=1/capture-presets=1/editor-workflow=1/asset-pipeline=1/rendering-advanced=1/runtime-sequencer=1/audio-production-feature=1/production-publishing=1 coverage, and local performance history rows for all six suites against `dist\DISPARITY-Release\DisparityGame.exe`.
...
- The v28 pass was verified with `Tools/VerifyDisparity.ps1`; run `git status --short` and review the signed commit output before pushing.
