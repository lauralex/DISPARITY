...
- `DisparityGame/Source/DisparityGame.cpp`: first third-person walking prototype, public-demo shard/extraction loop, editor viewport diagnostics HUD, persisted editor preferences, viewport toolbar overlay, runtime verification scenario coverage, showcase/trailer mode orchestration, and high-resolution capture resolve workflow.
...
- `Tools/ReviewVerificationBaselines.ps1`: checks baseline assets for required capture/render-graph/editor-target/golden/v24-v29 coverage fields and runs performance summaries.
- `Tools/ReviewProductionBatch.ps1`: validates the v25 forty-point production followup manifest, v28 thirty-six-point diversified followup manifest, and v29 thirty-point public-demo manifest, then writes review JSON under `Saved/Verification`.
...
- `F10`: reset the public playable demo loop, returning rift shards, sentinels, extraction state, and HUD objectives to their start state.
- Hold `Shift` while using `WASD` in gameplay to sprint through the public demo loop.
...
On 2026-04-27 after the v29 public playable demo batch:
...
- `Tools/RuntimeVerifyDisparity.ps1 -Configuration Debug -Frames 96 -DisableGoldenComparison` completed successfully and reported `version=0.29.0`, `public_demo_tests=1`, `public_demo_shard_pickups=6`, `public_demo_completions=1`, `public_demo_hud_frames>0`, `public_demo_beacon_draws>0`, `v29_public_demo_points=30`, `production_schema_metrics=207`, and one passing `v29_point_*` metric for every public-demo followup point.
- `Tools/VerifyDisparity.ps1` completed successfully for v29; it now includes assetized runtime report schema assertions, ten v24 production-batch counters, forty v25 production followup point counters, v26 editor-preference persistence and viewport-toolbar counters, v27 named profile/capture-preset/VFX-emitter/cooked-dependency counters, v28 diversified category/point counters, v29 public-demo shard/HUD/completion/point counters, viewport-HUD/debug-thumbnail baseline coverage, transform precision checks, command-history filtering checks, v6 Shot Director metadata checks, VFX profile checks, cooked GPU-resource readiness checks, dependency invalidation/preview/rollback/streaming/skinning/retargeting checks, audio calibration and production-feature checks, production followup manifest review, release-readiness review, cooked package review, Git-signature-aware baseline approval metadata checks, signed baseline update approval intent, trailer launch/OBS plan generation, symbol publishing, installer bootstrapper generation, and median-aware performance trend review.
...
- `Tools/ReviewVerificationBaselines.ps1 -ListGoldenProfiles` validated required baseline keys, including all v24 production-batch counters, `min_v25_production_points=40`, `min_editor_preference_persistence_tests=1`, `min_viewport_toolbar_tests=1`, the v27 profile/capture/VFX/cooked-dependency gates, the v28 category gates plus `min_v28_diversified_points=36`, and the v29 public-demo gates plus `min_v29_public_demo_points=30`, printed available golden profiles, and recognized the committed default plus known adapter profile set.
- `Tools/ReviewProductionBatch.ps1` validated `Assets/Verification/V25ProductionBatch.dfollowups` as exactly forty uniquely named v25 points, `Assets/Verification/V28DiversifiedBatch.dfollowups` as exactly thirty-six uniquely named v28 points, and `Assets/Verification/V29PublicDemo.dfollowups` as exactly thirty uniquely named v29 points, then wrote review JSON under `Saved/Verification`.
...
- `Tools/ReviewReleaseReadiness.ps1` produces `Saved/Release/release_readiness_manifest.json` and validates package, installer/bootstrapper, symbol, OBS, runtime schema, v25/v28 production batch artifacts, and the v29 public-demo manifest.
...
- `Tools/VerifyDisparity.ps1` produced PASS packaged runtime reports, captures, golden comparisons, showcase/trailer/high-res/VFX/beat/audio-analysis/Shot-Director/async-IO/animation-skinning/render-graph-callback/GPU-pick-histogram/cooked-package/nested-prefab/audio-production/viewport-overlay/high-resolve/viewport-HUD/transform-precision/command-history/schema/shot-sequencer/VFX-profile/VFX-emitter/cooked-GPU/dependency-invalidation/cooked-dependency-preview/audio-calibration/release-readiness/v25=40/v28=36/v29=30/demo=1/pickups=6/completions=1/prefs=1/toolbar=1/profiles=1/capture-presets=1/editor-workflow=1/asset-pipeline=1/rendering-advanced=1/runtime-sequencer=1/audio-production-feature=1/production-publishing=1 coverage, and local performance history rows for all six suites against `dist\DISPARITY-Release\DisparityGame.exe`.
...
- The v29 pass was verified with `Tools/VerifyDisparity.ps1`; run `git status --short` and review the signed commit output before pushing.
...
- Turn the v29 public demo loop into a richer vertical slice with a chained objective, light traversal puzzle, checkpoint/fail/retry states, gamepad input, animation-state placeholders, and stronger menu-to-gameplay flow.
- Replace placeholder pickup/completion feedback with real XAudio2-backed sounds, footsteps, spatial sentinel cues, content-driven rift pulses, and presentation-safe mixer routing.
