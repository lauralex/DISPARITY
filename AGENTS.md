...
- `Tools/ReviewReleaseReadiness.ps1`: validates package, installer/bootstrapper, symbols, OBS profile, runtime-report schema manifests, and v25/v28/v29/v30/v31/v32 followup manifests, then writes `Saved/Release/release_readiness_manifest.json`.
...
- `F10`: reset the public playable demo loop, returning rift shards, phase anchors, resonance gates, phase relays, checkpoint/retry state, sentinels, extraction state, and HUD objectives to their start state.
- Hold `Shift` while using `WASD` in gameplay to sprint through the shard/anchor/resonance-gate/phase-relay/extraction public demo loop.
...
On 2026-04-27 after the v32 sixty-point roadmap batch:
...
- `Tools/RuntimeVerifyDisparity.ps1 -Configuration Debug -Frames 96 -DisableGoldenComparison` completed successfully and reported `version=0.32.0`, `public_demo_tests=1`, `public_demo_shard_pickups>=6`, `public_demo_anchor_activations>=3`, `public_demo_resonance_gates>=2`, `public_demo_phase_relays>=3`, `public_demo_relay_overcharge_windows>=1`, `public_demo_combo_steps>=12`, `public_demo_completions=1`, `v29_public_demo_points=30`, `v30_vertical_slice_points=36`, `v31_diversified_points=30`, `v32_roadmap_points=60`, `production_schema_metrics=373`, and one passing `v32_point_*` metric for every roadmap followup point.
- `Tools/VerifyDisparity.ps1` completed successfully for v32; it now includes assetized runtime report schema assertions, v24-v32 followup counters, viewport-HUD/debug-thumbnail baseline coverage, transform precision checks, command-history filtering checks, v6 Shot Director metadata checks, VFX profile checks, cooked GPU-resource readiness checks, dependency invalidation/preview/rollback/streaming/skinning/retargeting checks, audio calibration and production-feature checks, production followup manifest review, release-readiness review, cooked package review, Git-signature-aware baseline approval metadata checks, signed baseline update approval intent, trailer launch/OBS plan generation, symbol publishing, installer bootstrapper generation, and median-aware performance trend review.
...
- `Tools/ReviewVerificationBaselines.ps1 -ListGoldenProfiles` validated required baseline keys, including all v24 production-batch counters, the v25-v31 gates, `min_v32_roadmap_points=60`, `min_public_demo_phase_relays=3`, `min_public_demo_relay_overcharge_windows=1`, and `min_public_demo_combo_steps=1`, printed available golden profiles, and recognized the committed default plus known adapter profile set.
- `Tools/ReviewProductionBatch.ps1` validated `Assets/Verification/V25ProductionBatch.dfollowups`, `Assets/Verification/V28DiversifiedBatch.dfollowups`, `Assets/Verification/V29PublicDemo.dfollowups`, `Assets/Verification/V30VerticalSlice.dfollowups`, `Assets/Verification/V31DiversifiedBatch.dfollowups`, and `Assets/Verification/V32RoadmapBatch.dfollowups` with expected counts, unique names, domains, and descriptions, then wrote review JSON under `Saved/Verification`.
...
- The v32 pass was verified with `Tools/VerifyDisparity.ps1`; run `git status --short` and review the signed commit output before pushing.
...
- Turn the v32 relay route into a more game-like public demo with collision-backed traversal, controller/gamepad gameplay, a title/menu-to-gameplay flow, simple enemy behaviors, an authored failure/retry screen, and richer objective-gate affordances.
- Replace placeholder pickup/anchor/resonance/completion feedback with real XAudio2-backed sounds, authored footsteps, spatial sentinel cues, content-driven rift pulses, and presentation-safe mixer routing.
