...
On 2026-04-27 after the v25 forty-point production followup implementation pass:
...
- `Tools/ReviewVerificationBaselines.ps1 -ListGoldenProfiles` validated required baseline keys, including all v24 production-batch counters and `min_v25_production_points=40`, printed available golden profiles, and recognized the committed default plus known adapter profile set.
...
- `Tools/VerifyDisparity.ps1` produced PASS packaged runtime reports, captures, golden comparisons, showcase/trailer/high-res/VFX/beat/audio-analysis/Shot-Director/async-IO/animation-skinning/render-graph-callback/GPU-pick-histogram/cooked-package/nested-prefab/audio-production/viewport-overlay/high-resolve/viewport-HUD/transform-precision/command-history/schema/shot-sequencer/VFX-profile/cooked-GPU/dependency-invalidation/audio-calibration/release-readiness/v25=40 coverage, and local performance history rows for all six suites against `dist\DISPARITY-Release\DisparityGame.exe`.
...
- The v25 pass was verified with `Tools/VerifyDisparity.ps1`; run `git status --short` and review the signed commit output before pushing.
...
- Turn the v25 editor preference readiness profile into actual load/save of viewport HUD rows, transform precision, dock layout, command-history filters, and user-scoped editor settings.
- Build the v25 viewport toolbar readiness into click-through camera/render-debug/capture/object-ID/depth overlay controls directly on the dedicated ImGui viewport texture.
