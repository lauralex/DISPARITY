...
- `F10`: reset the public playable demo loop, returning rift shards, phase anchors, resonance gates, checkpoint/retry state, sentinels, extraction state, and HUD objectives to their start state.
- Hold `Shift` while using `WASD` in gameplay to sprint through the shard/anchor/resonance-gate/extraction public demo loop.
...
On 2026-04-27 after the v31 diversified roadmap batch:
...
- `Tools/RuntimeVerifyDisparity.ps1 -Configuration Debug -Frames 96 -DisableGoldenComparison` completed successfully and reported `version=0.31.0`, `public_demo_tests=1`, `public_demo_shard_pickups>=6`, `public_demo_anchor_activations>=3`, `public_demo_resonance_gates>=2`, `public_demo_pressure_hits>=1`, `public_demo_footstep_events>=1`, `public_demo_retries>=1`, `public_demo_checkpoints>=1`, `public_demo_director_frames>=1`, `public_demo_completions=1`, `v29_public_demo_points=30`, `v30_vertical_slice_points=36`, `v31_diversified_points=30`, `production_schema_metrics>=299`, and one passing `v31_point_*` metric for every diversified roadmap followup point.
- `Tools/VerifyDisparity.ps1` completed successfully for v31; it now includes assetized runtime report schema assertions, ten v24 production-batch counters, forty v25 production followup point counters, v26 editor-preference persistence and viewport-toolbar counters, v27 named profile/capture-preset/VFX-emitter/cooked-dependency counters, v28 diversified category/point counters, v29 public-demo shard/HUD/completion/point counters, v30 vertical-slice stage/anchor/retry/checkpoint/director/point counters, v31 resonance-gate/pressure/footstep/event-route/point counters, viewport-HUD/debug-thumbnail baseline coverage, transform precision checks, command-history filtering checks, v6 Shot Director metadata checks, VFX profile checks, cooked GPU-resource readiness checks, dependency invalidation/preview/rollback/streaming/skinning/retargeting checks, audio calibration and production-feature checks, production followup manifest review, release-readiness review, cooked package review, Git-signature-aware baseline approval metadata checks, signed baseline update approval intent, trailer launch/OBS plan generation, symbol publishing, installer bootstrapper generation, and median-aware performance trend review.
...
- `Tools/ReviewVerificationBaselines.ps1 -ListGoldenProfiles` validated required baseline keys, including all v24 production-batch counters, `min_v25_production_points=40`, `min_editor_preference_persistence_tests=1`, `min_viewport_toolbar_tests=1`, the v27 profile/capture/VFX/cooked-dependency gates, the v28 category gates plus `min_v28_diversified_points=36`, the v29 public-demo gates plus `min_v29_public_demo_points=30`, the v30 vertical-slice gates plus `min_v30_vertical_slice_points=36`, and the v31 diversified gates plus `min_v31_diversified_points=30`, printed available golden profiles, and recognized the committed default plus known adapter profile set.
- `Tools/ReviewProductionBatch.ps1` validated `Assets/Verification/V25ProductionBatch.dfollowups` as exactly forty uniquely named v25 points, `Assets/Verification/V28DiversifiedBatch.dfollowups` as exactly thirty-six uniquely named v28 points, `Assets/Verification/V29PublicDemo.dfollowups` as exactly thirty uniquely named v29 points, `Assets/Verification/V30VerticalSlice.dfollowups` as exactly thirty-six uniquely named v30 points, and `Assets/Verification/V31DiversifiedBatch.dfollowups` as exactly thirty uniquely named v31 points, then wrote review JSON under `Saved/Verification`.
...
- `Tools/ReviewReleaseReadiness.ps1` produces `Saved/Release/release_readiness_manifest.json` and validates package, installer/bootstrapper, symbol, OBS, runtime schema, v25/v28 production batch artifacts, the v29 public-demo manifest, the v30 vertical-slice manifest, and the v31 diversified roadmap manifest.
...
- `Tools/VerifyDisparity.ps1` produced PASS packaged runtime reports, captures, golden comparisons, showcase/trailer/high-res/VFX/beat/audio-analysis/Shot-Director/async-IO/animation-skinning/render-graph-callback/GPU-pick-histogram/cooked-package/nested-prefab/audio-production/viewport-overlay/high-resolve/viewport-HUD/transform-precision/command-history/schema/shot-sequencer/VFX-profile/VFX-emitter/cooked-GPU/dependency-invalidation/cooked-dependency-preview/audio-calibration/release-readiness/v25=40/v28=36/v29=30/v30=36/v31=30/demo=1/pickups>=6/anchors>=3/gates>=2/pressure>=1/footsteps>=1/retries>=1/completions=1/prefs=1/toolbar=1/profiles=1/capture-presets=1/editor-workflow=1/asset-pipeline=1/rendering-advanced=1/runtime-sequencer=1/audio-production-feature=1/production-publishing=1 coverage, and local performance history rows for all six suites against `dist\DISPARITY-Release\DisparityGame.exe`.
...
- The v31 pass was verified with `Tools/VerifyDisparity.ps1`; run `git status --short` and review the signed commit output before pushing.
...
- Turn the v31 vertical slice into a more game-like public demo with collision-backed traversal, controller/gamepad gameplay, a title/menu-to-gameplay flow, an authored failure/retry screen, and richer resonance-gate affordances.
