...
- `Tools/ReviewVerificationBaselines.ps1`: checks baseline assets for required capture/render-graph/editor-target/golden/v22 coverage fields and runs performance summaries.
...
- `F9`: queue a source PPM, source PNG, async 2x PPM photo capture, and high-resolution capture manifest under `Saved/Captures`.
...
On 2026-04-27 after the v22 production followup implementation pass:
...
- `Tools/TestImGuiIds.ps1` completed successfully after the Shot Director v5 controls were added and is wired into `Tools/VerifyDisparity.ps1`.
...
- `Tools/VerifyDisparity.ps1` completed successfully for v22; it now includes cooked package review, Git-signature-aware baseline approval metadata checks, signed baseline update approval intent, trailer launch/OBS plan generation, symbol publishing, and installer bootstrapper generation.
...
- The v22 pass was verified with `Tools/VerifyDisparity.ps1`; run `git status --short` and review the signed commit output before pushing.
...
- Promote hover cache and latency histogram diagnostics into viewport overlays with stale-readback age and object-name display.
- Promote graph-owned high-resolution capture diagnostics into true tiled supersampling, resolve filters, EXR output, and async compression workers.
