...
- Editor/runtime v7 includes docking, multi-viewport panels, undo/redo with command labels, selection outlines, viewport click-picking, Ctrl multi-select, copy/paste/duplicate/delete scene-object workflows, an independent editor camera, simple transform gizmo buttons plus draggable 3D translation handles with snapping, prefab apply/save workflows, visibly stronger post effects with debug views, bus-based audio controls, spatial tone preview, an asset database with import settings and cooked metadata files, a compiled render graph schedule with pass CPU/GPU timings, culled passes, transition diagnostics, alias candidates, resource lifetimes, job system, version reporting, crash logs, and CI/package verification scripts.
...
- Selected objects and the player draw colored 3D transform handles at the active selection pivot. Left-drag an X/Y/Z handle to translate the active selection; hold `Shift` while dragging to snap.
...
On 2026-04-26 after the v7 graph-diagnostics/gizmo-drag followup pass:
...
- The v7 pass was committed only after the above checks and `git status --short` review.
...
- Add a dedicated editor viewport render target, object-ID selection buffer, and rotate/scale/local-world modes for the visible 3D gizmo handles.
