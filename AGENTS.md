...
- `Tools/ReviewReleaseReadiness.ps1`: validates package, installer/bootstrapper, symbols, OBS profile, runtime-report schema manifests, and v25/v28/v29/v30/v31/v32/v33 followup manifests, then writes `Saved/Release/release_readiness_manifest.json`.
...
On 2026-04-27 after the v33 playable-demo batch:
...
- `Tools/RuntimeVerifyDisparity.ps1 -Configuration Debug -Frames 96 -DisableGoldenComparison` completed successfully and reported `version=0.33.0`, `public_demo_tests=1`, `public_demo_shard_pickups>=6`, `public_demo_anchor_activations>=3`, `public_demo_resonance_gates>=2`, `public_demo_phase_relays>=3`, `public_demo_collision_solves>=1`, `public_demo_traversal_actions>=1`, `public_demo_enemy_chases>=1`, `public_demo_enemy_evades>=1`, `public_demo_gamepad_frames>=1`, `public_demo_menu_transitions>=1`, `public_demo_failure_presentations>=1`, `public_demo_content_audio_cues>=1`, `public_demo_animation_state_changes>=1`, `public_demo_completions=1`, `v29_public_demo_points=30`, `v30_vertical_slice_points=36`, `v31_diversified_points=30`, `v32_roadmap_points=60`, `v33_playable_demo_points=50`, `production_schema_metrics=449`, and one passing `v33_point_*` metric for every playable-demo followup point.
- `Tools/VerifyDisparity.ps1` completed successfully for v33; it now includes assetized runtime report schema assertions, v24-v33 followup counters, viewport-HUD/debug-thumbnail baseline coverage, transform precision checks, command-history filtering checks, v6 Shot Director metadata checks, VFX profile checks, cooked GPU-resource readiness checks, dependency invalidation/preview/rollback/streaming/skinning/retargeting checks, audio calibration and production-feature checks, production followup manifest review, release-readiness review, cooked package review, Git-signature-aware baseline approval metadata checks, signed baseline update approval intent, trailer launch/OBS plan generation, symbol publishing, installer bootstrapper generation, packaged runtime suites, and median-aware performance trend review.
...
- `Tools/ReviewVerificationBaselines.ps1 -ListGoldenProfiles` validated required baseline keys, including all v24 production-batch counters, the v25-v32 gates, `min_v33_playable_demo_points=50`, `min_public_demo_collision_solves=1`, `min_public_demo_traversal_actions=1`, `min_public_demo_enemy_chases=1`, `min_public_demo_enemy_evades=1`, `min_public_demo_gamepad_frames=1`, `min_public_demo_menu_transitions=1`, `min_public_demo_failure_presentations=1`, `min_public_demo_content_audio_cues=1`, and `min_public_demo_animation_state_changes=1`, printed available golden profiles, and recognized the committed default plus known adapter profile set.
- `Tools/ReviewProductionBatch.ps1` validated `Assets/Verification/V25ProductionBatch.dfollowups`, `Assets/Verification/V28DiversifiedBatch.dfollowups`, `Assets/Verification/V29PublicDemo.dfollowups`, `Assets/Verification/V30VerticalSlice.dfollowups`, `Assets/Verification/V31DiversifiedBatch.dfollowups`, `Assets/Verification/V32RoadmapBatch.dfollowups`, and `Assets/Verification/V33PlayableDemoBatch.dfollowups` with expected counts, unique names, domains, and descriptions, then wrote review JSON under `Saved/Verification`.
...
- The v33 pass was verified with `Tools/VerifyDisparity.ps1`; run `git status --short` and review the signed commit output before pushing.
...
- Replace the v33 manifest-level animation surface with authored clips, blend trees, animation events, root-motion experiments, and transition previews in the editor.
...
- Replace the audio production/calibration diagnostics with real XAudio2 mixer voices, streamed music assets, spatial emitter components, attenuation-curve assets, calibrated meters, and content-driven VFX pulses.
