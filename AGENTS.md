...
- `Tools/ReviewVerificationBaselines.ps1`: checks baseline assets for required capture/render-graph/editor-target/golden/v24-v30 coverage fields and runs performance summaries.
- `Tools/ReviewProductionBatch.ps1`: validates the v25 forty-point production followup manifest, v28 thirty-six-point diversified followup manifest, v29 thirty-point public-demo manifest, and v30 thirty-six-point vertical-slice manifest, then writes review JSON under `Saved/Verification`.
...
- `F10`: reset the public playable demo loop, returning rift shards, phase anchors, checkpoint/retry state, sentinels, extraction state, and HUD objectives to their start state.
- Hold `Shift` while using `WASD` in gameplay to sprint through the shard/anchor/extraction public demo loop.
...
On 2026-04-27 after the v30 public vertical-slice batch:
...
- `Tools/ReviewReleaseReadiness.ps1` produces `Saved/Release/release_readiness_manifest.json` and validates package, installer/bootstrapper, symbol, OBS, runtime schema, v25/v28 production batch artifacts, the v29 public-demo manifest, and the v30 vertical-slice manifest.
...
- `Tools/VerifyDisparity.ps1` produced PASS packaged runtime reports, captures, golden comparisons, showcase/trailer/high-res/VFX/beat/audio-analysis/Shot-Director/async-IO/animation-skinning/render-graph-callback/GPU-pick-histogram/cooked-package/nested-prefab/audio-production/viewport-overlay/high-resolve/viewport-HUD/transform-precision/command-history/schema/shot-sequencer/VFX-profile/VFX-emitter/cooked-GPU/dependency-invalidation/cooked-dependency-preview/audio-calibration/release-readiness/v25=40/v28=36/v29=30/v30=36/demo=1/pickups>=6/anchors>=3/retries>=1/completions=1/prefs=1/toolbar=1/profiles=1/capture-presets=1/editor-workflow=1/asset-pipeline=1/rendering-advanced=1/runtime-sequencer=1/audio-production-feature=1/production-publishing=1 coverage, and local performance history rows for all six suites against `dist\DISPARITY-Release\DisparityGame.exe`.
...
- The v30 pass was verified with `Tools/VerifyDisparity.ps1`; run `git status --short` and review the signed commit output before pushing.
...
- Turn the v30 vertical slice into a more game-like public demo with collision-backed traversal, controller/gamepad gameplay, a title/menu-to-gameplay flow, a failure/retry screen, and richer objective-gate affordances.
- Replace placeholder pickup/anchor/completion feedback with real XAudio2-backed sounds, footsteps, spatial sentinel cues, content-driven rift pulses, and presentation-safe mixer routing.
