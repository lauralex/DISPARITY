...
- `DisparityEngine/Source/Disparity/Audio/`: Windows audio hooks with bus/meter/snapshot/analysis surfaces, dynamic XAudio2 generated-tone playback when available, and WinMM fallback output.
...
- `Assets/Cinematics/Showcase.dshot`: authored v4 trailer/photo camera keys with spline, timeline-lane, and thumbnail metadata for repeatable public capture.
...
- `F9`: queue a source PPM, source PNG, and async 2x PPM photo capture under `Saved/Captures`.
...
On 2026-04-27 after the v21 production followup implementation pass:
...
- `Tools/TestImGuiIds.ps1` completed successfully after the Shot Director v4 controls were added and is wired into `Tools/VerifyDisparity.ps1`.
...
- `Tools/VerifyDisparity.ps1` completed successfully for v21; it now includes optimized `.dglbpack` schema review, Git-signature-aware baseline approval metadata checks, and interactive CI plan generation.
...
- The v21 pass was verified with `Tools/VerifyDisparity.ps1`; run `git status --short` and review the signed commit output before pushing.
...
- Add hover cache visualization and latency histograms for the async GPU picking ring.
- Replace the current async 2x PPM worker with graph-owned offscreen high-resolution render targets, MSAA resolve options, and true tiled supersampling.
- Expand v4 Shot Director metadata into editable easing curves, renderer/audio cue timeline tracks, generated shot thumbnails, and non-modal preview scrubbing.
- Promote the VFX stats/simulation surface into a dedicated particle/ribbon renderer with soft particles, depth fade, sorting controls, real GPU simulation, motion vectors, and temporal VFX reprojection.
...
- Load `DSGLBPK2` packages as optimized cooked mesh/material/animation resources, add runtime dependency invalidation, retargeting, and real GPU skinning palette uploads.
...
- Expand the XAudio2 generated-tone path into real mixer voices, streamed music, spatial emitters, attenuation curves, production meters, and real content-driven VFX pulses.
- Replace bootstrapper/symbol-server plans with actual publishing/install artifacts, run packaged runtime smoke tests on interactive CI runners, and expand trailer launch presets into OBS scene/profile automation.
