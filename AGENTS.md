...
- `Tools/ReviewVerificationBaselines.ps1`: checks baseline assets for required capture/render-graph/editor-target/golden/v24-v34 coverage fields and runs performance summaries.
- `Tools/ReviewProductionBatch.ps1`: validates the v25 forty-point production followup manifest, v28 thirty-six-point diversified followup manifest, v29 thirty-point public-demo manifest, v30 thirty-six-point vertical-slice manifest, v31 thirty-point diversified roadmap manifest, v32 sixty-point roadmap manifest, v33 fifty-point playable-demo manifest, and v34 sixty-point AAA-foundation manifest, then writes review JSON under `Saved/Verification`.
...
- `Tools/ReviewReleaseReadiness.ps1`: validates package, installer/bootstrapper, symbols, OBS profile, runtime-report schema manifests, and v25/v28/v29/v30/v31/v32/v33/v34 followup manifests, then writes `Saved/Release/release_readiness_manifest.json`.
...
On 2026-04-28 after the v34 AAA-foundation batch:
...
- `Tools/VerifyDisparity.ps1` produced PASS packaged runtime reports, captures, golden comparisons, showcase/trailer/high-res/VFX/beat/audio-analysis/Shot-Director/async-IO/animation-skinning/render-graph-callback/GPU-pick-histogram/cooked-package/nested-prefab/audio-production/viewport-overlay/high-resolve/viewport-HUD/transform-precision/command-history/schema/shot-sequencer/VFX-profile/VFX-emitter/cooked-GPU/dependency-invalidation/cooked-dependency-preview/audio-calibration/release-readiness/v25=40/v28=36/v29=30/v30=36/v31=30/v32=60/v33=50/v34=60/demo=1/pickups>=6/anchors>=3/gates>=2/relays>=3/collision>=1/traversal>=1/enemy>=1/enemy-archetypes=3/los>=1/telegraphs>=1/hit-reactions>=1/controller>=1/blend>=1/gamepad>=1/menu>=1/failure>=1/cues>=1/animation>=1/overcharge>=1/combo>=12/pressure>=1/footsteps>=1/retries>=1/completions=1/prefs=1/toolbar=1/profiles=1/capture-presets=1/editor-workflow=1/asset-pipeline=1/rendering-advanced=1/runtime-sequencer=1/audio-production-feature=1/production-publishing=1 coverage, and local performance history rows for all six suites against `dist\DISPARITY-Release\DisparityGame.exe`.
...
- The v34 pass was verified with `Tools/VerifyDisparity.ps1`; run `git status --short` and review the signed commit output before pushing.
