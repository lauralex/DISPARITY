...
- `F9`: queue a 2x PPM photo capture to `Saved/Captures/DISPARITY_photo_2x.ppm`.
...
- Replace the 2x PPM high-resolution capture proof with graph-owned offscreen render targets, MSAA resolve options, WIC/PNG export, tiled supersampling, and async capture workers.
- Add a shot-director editor for `.dshot` files with camera splines, easing curves, renderer/audio cue tracks, shot thumbnails, bookmarks, and preview scrubbing.
...
- Replace installer manifests with a real bootstrapper, publish symbols to a symbol server, add authenticated crash upload retries, run packaged runtime smoke tests on interactive CI runners, and add repeatable OBS/trailer launch presets.
