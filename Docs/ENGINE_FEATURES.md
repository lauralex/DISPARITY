...
- The `Profiler` panel includes production readiness tables for the forty v25 points and the thirty-six v28 diversified points, making editor workflow, asset promotion, rendering, runtime sequencer, audio production, and publishing coverage inspectable while the build is running.
...
- `Tools/SummarizePerformanceHistory.ps1` groups recent local runs by suite/executable and reports CPU/GPU deltas plus editor/gizmo/showcase/trailer/high-res/VFX/beat/viewport-overlay/high-resolve/schema/release/editor-preference/viewport-toolbar/profile/capture-preset/VFX-emitter/cooked-dependency/v28-category scenario counters, then compares them against committed performance baselines when available. Regression checks compare both the previous run and the recent median so isolated max-frame spikes remain visible without immediately failing the gate.
- `Tools/ReviewVerificationBaselines.ps1` checks runtime baselines for required capture, graph, editor target, golden, v24-v28 coverage fields, and v28 category gates before running the performance summary.
- `Tools/ReviewProductionBatch.ps1` checks both the v25 followup manifest and the v28 diversified manifest for expected counts, unique point names, domains, and descriptions, then writes `Saved/Verification/v25_production_batch_review.json` and `Saved/Verification/v28_diversified_batch_review.json`.
