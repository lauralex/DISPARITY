...
- Public-demo visuals include shard beacons, phase-anchor glyph rings, resonance-gate rings/glyphs, phase-relay rings/glyphs/orbit shards, bridge beams from aligned anchors, tuned gates, and stabilized relays to the rift, relay overcharge warning rings, route markers, traversal barriers, enemy vision rings and chase beams, a checkpoint marker, low-stability warning rings, extraction gate rings, sentinel hazard columns, stronger rift charge as progress builds, and a completion pulse that remains visible in showcase/trailer captures.
- Use the `Demo Director` panel to inspect current stage, objective distance, shard/anchor/gate/relay progress, checkpoint/retry counters, pressure-hit/footstep/combo/collision/traversal/enemy/gamepad/menu/content-audio/animation telemetry, recent gameplay events, and v30/v31/v32/v33 readiness. It also exposes reset, save-checkpoint, and retry controls for recording/debug passes.
- Runtime verification exercises a deterministic full route, collects all shards, activates all phase anchors, tunes all resonance gates, stabilizes all phase relays, solves collision, triggers traversal, simulates gamepad/menu frames, forces enemy chase/evade/contact and checkpoint retry behavior, records pressure-hit, footstep cadence, overcharge-window, content audio, animation, failure-presentation, and combo-route telemetry, completes extraction, draws the Demo Director/readiness surface, and reports `public_demo_tests`, `public_demo_shard_pickups`, `public_demo_completions`, `public_demo_hud_frames`, `public_demo_beacon_draws`, `public_demo_objective_stages`, `public_demo_anchor_activations`, `public_demo_resonance_gates`, `public_demo_phase_relays`, `public_demo_relay_overcharge_windows`, `public_demo_combo_steps`, `public_demo_collision_solves`, `public_demo_traversal_actions`, `public_demo_enemy_chases`, `public_demo_enemy_evades`, `public_demo_gamepad_frames`, `public_demo_menu_transitions`, `public_demo_failure_presentations`, `public_demo_content_audio_cues`, `public_demo_animation_state_changes`, `public_demo_pressure_hits`, `public_demo_footstep_events`, `public_demo_retries`, `public_demo_checkpoints`, `public_demo_director_frames`, `public_demo_stability`, `public_demo_focus`, all thirty `v29_point_*` metrics, all thirty-six `v30_point_*` metrics, all thirty `v31_point_*` metrics, all sixty `v32_point_*` metrics, and all fifty `v33_point_*` metrics.
