...
- The `Profiler` panel includes production readiness tables for the forty v25 points, thirty-six v28 diversified points, thirty v29 public-demo points, thirty-six v30 vertical-slice points, thirty v31 diversified roadmap points, and sixty v32 roadmap points, making editor workflow, asset promotion, rendering, runtime sequencer, audio production, publishing, and playable-demo coverage inspectable while the build is running.

## Playable Public Demo

- The default prototype now has a compact playable DISPARITY vertical slice that can be shown without entering editor panels: collect all six glowing rift shards, align three phase anchors, tune two resonance gates, stabilize three phase relays during overcharge windows, stay away from sentinel pressure, retry from checkpoints when stability collapses, and return to the extraction beacon to finish the objective.
- Use `WASD` to move and hold `Shift` to sprint. The HUD in the top-left shows stage, shard progress, phase-anchor progress, resonance-gate progress, phase-relay progress, stability, focus, checkpoint/retry state, objective distance, extraction readiness, completion state, and the current objective text.
- Press `F10` or use `DISPARITY > Reset Public Demo` to reset shard pickups, phase anchors, resonance gates, phase relays, checkpoint/retry state, sentinel pressure, extraction state, rift surge, and objective status while the game is running.
- Public-demo visuals include shard beacons, phase-anchor glyph rings, resonance-gate rings/glyphs, phase-relay rings/glyphs/orbit shards, bridge beams from aligned anchors, tuned gates, and stabilized relays to the rift, relay overcharge warning rings, route markers, a checkpoint marker, low-stability warning rings, extraction gate rings, sentinel hazard columns, stronger rift charge as progress builds, and a completion pulse that remains visible in showcase/trailer captures.
- Use the `Demo Director` panel to inspect current stage, objective distance, shard/anchor/gate/relay progress, checkpoint/retry counters, pressure-hit/footstep/combo telemetry, recent gameplay events, and v30/v31/v32 readiness. It also exposes reset, save-checkpoint, and retry controls for recording/debug passes.
- Runtime verification exercises a deterministic full route, collects all shards, activates all phase anchors, tunes all resonance gates, stabilizes all phase relays, forces checkpoint retry behavior, records pressure-hit, footstep cadence, overcharge-window, and combo-route telemetry, completes extraction, draws the Demo Director/readiness surface, and reports `public_demo_tests`, `public_demo_shard_pickups`, `public_demo_completions`, `public_demo_hud_frames`, `public_demo_beacon_draws`, `public_demo_objective_stages`, `public_demo_anchor_activations`, `public_demo_resonance_gates`, `public_demo_phase_relays`, `public_demo_relay_overcharge_windows`, `public_demo_combo_steps`, `public_demo_pressure_hits`, `public_demo_footstep_events`, `public_demo_retries`, `public_demo_checkpoints`, `public_demo_director_frames`, `public_demo_stability`, `public_demo_focus`, all thirty `v29_point_*` metrics, all thirty-six `v30_point_*` metrics, all thirty `v31_point_*` metrics, and all sixty `v32_point_*` metrics.
