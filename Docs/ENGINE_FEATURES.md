...
- Editor preferences autosave under `Saved/Editor/editor_preferences.json`. They remember viewport HUD rows/pinning, viewport-toolbar visibility, editor-camera enablement, transform precision/pivot/orientation, the Profiler command-history filter, and the active named profile. Use `Viewport > Preference Profiles` to save/load sanitized profile files under `Saved/Editor/Profiles`, or reset the current editor preference surface to defaults.
