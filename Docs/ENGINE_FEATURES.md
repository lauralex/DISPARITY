...
- The `Profiler` panel includes production readiness tables for the forty v25 points, thirty-six v28 diversified points, thirty v29 public-demo points, and thirty-six v30 vertical-slice points, making editor workflow, asset promotion, rendering, runtime sequencer, audio production, publishing, and playable-demo coverage inspectable while the build is running.

## Playable Public Demo

- The default prototype now has a compact playable DISPARITY vertical slice that can be shown without entering editor panels: collect all six glowing rift shards, align three phase anchors, stay away from sentinel pressure, retry from checkpoints when stability collapses, and return to the extraction beacon to finish the objective.
- Use `WASD` to move and hold `Shift` to sprint. The HUD in the top-left shows stage, shard progress, phase-anchor progress, stability, focus, checkpoint/retry state, objective distance, extraction readiness, completion state, and the current objective text.
- Press `F10` or use `DISPARITY > Reset Public Demo` to reset shard pickups, phase anchors, checkpoint/retry state, sentinel pressure, extraction state, rift surge, and objective status while the game is running.
- Public-demo visuals include shard beacons, phase-anchor glyph rings, bridge beams from aligned anchors to the rift, route markers, a checkpoint marker, low-stability warning rings, extraction gate rings, sentinel hazard columns, stronger rift charge as progress builds, and a completion pulse that remains visible in showcase/trailer captures.
- Use the `Demo Director` panel to inspect current stage, objective distance, shard/anchor progress, checkpoint/retry counters, recent gameplay events, and v30 readiness. It also exposes reset, save-checkpoint, and retry controls for recording/debug passes.
- Runtime verification exercises a deterministic full route, collects all shards, activates all phase anchors, forces checkpoint retry behavior, completes extraction, draws the Demo Director/readiness surface, and reports `public_demo_tests`, `public_demo_shard_pickups`, `public_demo_completions`, `public_demo_hud_frames`, `public_demo_beacon_draws`, `public_demo_objective_stages`, `public_demo_anchor_activations`, `public_demo_retries`, `public_demo_checkpoints`, `public_demo_director_frames`, `public_demo_stability`, `public_demo_focus`, all thirty `v29_point_*` metrics, and all thirty-six `v30_point_*` metrics.
...
Release readiness review checks the assembled package, installer/bootstrapper output, symbols, OBS profile, runtime schema manifest, the v25/v28 production batch manifests, the v29 public-demo manifest, and the v30 vertical-slice manifest:
...
- `Tools/SummarizePerformanceHistory.ps1` groups recent local runs by suite/executable and reports CPU/GPU deltas plus editor/gizmo/showcase/trailer/high-res/VFX/beat/viewport-overlay/high-resolve/schema/release/editor-preference/viewport-toolbar/profile/capture-preset/VFX-emitter/cooked-dependency/v28-category/v29-public-demo/v30-vertical-slice scenario counters, then compares them against committed performance baselines when available. Regression checks compare both the previous run and the recent median so isolated max-frame spikes remain visible without immediately failing the gate.
- `Tools/ReviewVerificationBaselines.ps1` checks runtime baselines for required capture, graph, editor target, golden, v24-v30 coverage fields, v28 category gates, v29 public-demo gates, and v30 vertical-slice gates before running the performance summary.
- `Tools/ReviewProductionBatch.ps1` checks the v25 followup manifest, v28 diversified manifest, v29 public-demo manifest, and v30 vertical-slice manifest for expected counts, unique point names, domains, and descriptions, then writes review JSON under `Saved/Verification`.
