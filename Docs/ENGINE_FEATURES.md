...
- The `Profiler` panel includes production readiness tables for the forty v25 points, thirty-six v28 diversified points, and thirty v29 public-demo points, making editor workflow, asset promotion, rendering, runtime sequencer, audio production, publishing, and playable-demo coverage inspectable while the build is running.

## Playable Public Demo

- The default prototype now has a compact playable DISPARITY loop that can be shown without entering editor panels: collect all six glowing rift shards, stay away from sentinel pressure, and return to the extraction beacon to finish the objective.
- Use `WASD` to move and hold `Shift` to sprint. The HUD in the top-left shows shard progress, stability, focus, extraction readiness, completion state, and the current objective text.
- Press `F10` or use `DISPARITY > Reset Public Demo` to reset shard pickups, sentinel pressure, extraction state, rift surge, and objective status while the game is running.
- Public-demo visuals include shard beacons, route markers, extraction gate rings, sentinel hazard columns, stronger rift charge when shards are collected, and a completion pulse that remains visible in showcase/trailer captures.
- Runtime verification exercises a deterministic full route, collects all shards, completes extraction, and reports `public_demo_tests`, `public_demo_shard_pickups`, `public_demo_completions`, `public_demo_hud_frames`, `public_demo_beacon_draws`, `public_demo_stability`, `public_demo_focus`, and all thirty `v29_point_*` metrics.
...
Release readiness review checks the assembled package, installer/bootstrapper output, symbols, OBS profile, runtime schema manifest, the v25/v28 production batch manifests, and the v29 public-demo manifest:
...
- `Tools/SummarizePerformanceHistory.ps1` groups recent local runs by suite/executable and reports CPU/GPU deltas plus editor/gizmo/showcase/trailer/high-res/VFX/beat/viewport-overlay/high-resolve/schema/release/editor-preference/viewport-toolbar/profile/capture-preset/VFX-emitter/cooked-dependency/v28-category/v29-public-demo scenario counters, then compares them against committed performance baselines when available. Regression checks compare both the previous run and the recent median so isolated max-frame spikes remain visible without immediately failing the gate.
- `Tools/ReviewVerificationBaselines.ps1` checks runtime baselines for required capture, graph, editor target, golden, v24-v29 coverage fields, v28 category gates, and v29 public-demo gates before running the performance summary.
- `Tools/ReviewProductionBatch.ps1` checks the v25 followup manifest, v28 diversified manifest, and v29 public-demo manifest for expected counts, unique point names, domains, and descriptions, then writes review JSON under `Saved/Verification`.
